    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
        <Stat label="Pedidos hoje" value={String(todayOrders?.n ?? 0)}
          ...
          color="#F26B1F" href="/admin/pedidos" />
        ...
        <Stat label="Aguardando pagamento" value={String(pendingOrders?.n ?? 0)}
          color="#F59E0B" href="/admin/pedidos" />
        <Stat label="Para despachar" value={String(processingOrders?.n ?? 0)}
          color="#3B82F6" href="/admin/pedidos" />
        <Stat label="Enviados" value={String(shippedOrders?.n ?? 0)} color="#1FB5A8" href="/admin/pedidos" />
        <Stat label="Total pedidos" value={String(totalOrders?.n ?? 0)} href="/admin/pedidos" />
        <Stat label="Clientes" value={String(totalCustomers?.n ?? 0)} href="/admin/clientes" />
        <Stat label="Stock baixo (≤2)" value={String(lowStock?.n ?? 0)}
          color={lowStock?.n && lowStock.n > 0 ? '#E23B3B' : '#2CB35A'} href="/admin/produtos" />
      </div>
      ...
      <div style={{ background: '#0F1318', border: '1px solid #1E2530', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #1E2530', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Link href="/admin/pedidos" style={{ fontSize: 13, color: '#F26B1F', textDecoration: 'none' }}>Ver todos →</Link>
        </div>
