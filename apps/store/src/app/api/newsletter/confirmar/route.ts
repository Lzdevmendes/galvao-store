...
// LGPD double opt-in: confirma a inscrição a partir do link enviado por e-mail.
export async function GET(req: NextRequest) {
  const token = req.nextUrl.searchParams.get('token') ?? ''
  if (!token) return html('Link inválido', 'Este link de confirmação não é válido.')
  ...
  if (res.rowsAffected === 0) {
    return html('Já confirmado', 'Esta inscrição já estava confirmada ou o link expirou.')
  }
  return html('Inscrição confirmada! ⚽', 'Pronto! Você agora faz parte da lista VIP da Galvão\'s Store.')
}
...
<body style="font-family:system-ui,sans-serif;background:#0B0E12;color:#F8F9FB;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0;padding:24px">
  <div style="max-width:440px;text-align:center;background:#13171d;border:1px solid #232a33;border-radius:16px;padding:40px 32px">
    ...
    <a href="${APP_URL}/produtos" style="display:inline-block;background:#F26B1F;color:#fff;text-decoration:none;font-weight:700;padding:12px 28px;border-radius:8px">Ver produtos →</a>
  </div>
</body></html>`,
